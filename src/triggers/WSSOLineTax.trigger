trigger WSSOLineTax on PBSI__PBSI_Sales_Order_Line__c (before update, before insert, after insert, after update) {
    ...
    if(AllTriggersOff == FALSE && ThisTriggerOff == FALSE){
        ...
        List<PBSI__PBSI_Sales_Order_Line__c> lstSOLs6 = new List<PBSI__PBSI_Sales_Order_Line__c>();
        ...
        if(Trigger.isUpdate && Trigger.isAfter){
            ...
            for(PBSI__PBSI_Sales_Order_Line__c upSOL1 : trigger.new){

                PBSI__PBSI_Sales_Order_Line__c oldSOL = trigger.oldmap.get(upSOL1.id);

                if(upSOL1.PBSI__Item__c != oldSOL.PBSI__Item__c){//Do something if the item on the line changes
                    lstSOLs6.add(upSOL1); //Add the line to the list of changed lines
                }
        ...
        //Modify existing Price History Lines when the SO Line changes
        if(!lstSOLs6.isEmpty())WSSOLineTools.updateSOLineHistory(lstSOLs6);
